...
func (r *bucketIndexReader) fetchPostings(ctx context.Context, keys []labels.Label, bytesLimiter BytesLimiter, tenant string) ([]index.Postings, []func(), error) {
	...
	for ix, key := range keys {
		...
		if b, ok := fromCache[key]; ok {
			level.Info(r.logger).Log("msg", "fetch-postings", "from", "cache", "key", key.Name, "value", key.Value)
		...
		level.Info(r.logger).Log("msg", "fetch-postings", "from", "objstore", "key", key.Name, "value", key.Value)
...
func (r *bucketIndexReader) PreloadSeries(ctx context.Context, ids []storage.SeriesRef, bytesLimiter BytesLimiter, tenant string) error {
	...
	if !slices.IsSorted(ids) {
		level.Error(r.logger).Log("msg", "postings out of order", "series-refs", ids)
	}
...
func (r *bucketIndexReader) loadSeries(ctx context.Context, ids []storage.SeriesRef, refetch bool, start, end uint64, bytesLimiter BytesLimiter, tenant string) error {
	...
	for i, id := range ids {
		if int(uint64(id)-start) > len(b) {
			level.Error(r.logger).Log("block", r.block.meta.ULID.String(), "series-ref", id, "block-range-start", start, "block-range-length", end-start, "downloaded-buffer-size", len(b))
		}
		c := b[uint64(id)-start:]
